...
from seshtahvcode_py_cli import SeshtahvcodePyCli, DefaultAioHttpClient, AsyncSeshtahvcodePyCli
from seshtahvcode_py_cli._utils import is_dict
...
logging.getLogger("seshtahvcode_py_cli").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[SeshtahvcodePyCli]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with SeshtahvcodePyCli(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncSeshtahvcodePyCli]:
